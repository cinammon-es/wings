...
import (
	...
	"github.com/cinammon-net/wings/system"
)

const DefaultLocation = "/etc/cinammon/config.yml"
...
type SystemConfiguration struct {
	// The root directory where all of the cinammon data is stored at.
	RootDirectory string `default:"/var/lib/cinammon" json:"-" yaml:"root_directory"`

	// Directory where logs for server installations and other wings events are logged.
	LogDirectory string `default:"/var/log/cinammon" json:"-" yaml:"log_directory"`

	// Directory where the server data is stored at.
	Data string `default:"/var/lib/cinammon/volumes" json:"-" yaml:"data"`

	// Directory where server archives for transferring will be stored.
	ArchiveDirectory string `default:"/var/lib/cinammon/archives" json:"-" yaml:"archive_directory"`

	// Directory where local backups will be stored on the machine.
	BackupDirectory string `default:"/var/lib/cinammon/backups" json:"-" yaml:"backup_directory"`

	// TmpDirectory specifies where temporary files for cinammon installation processes
	// should be created. This supports environments running docker-in-docker.
	TmpDirectory string `default:"/tmp/cinammon" json:"-" yaml:"tmp_directory"`

	// The user that should own all of the server files, and be used for containers.
	Username string `default:"cinammon" yaml:"username"`
	...
	User struct {
		...
		PasswdFile string `json:"passwd_file" yaml:"passwd_file" default:"/etc/cinammon/passwd"`
	} `yaml:"user"`
...
type Configuration struct {
	...
	AppName string `default:"cinammon" json:"app_name" yaml:"app_name"`
...
// EnsurePelicanUser ensures that the Cinammon core user exists on the
...
func EnsurePelicanUser() error {
	...
	if sysName == "distroless" {
		_config.System.Username = system.FirstNotEmpty(os.Getenv("WINGS_USERNAME"), "cinammon")
	...
	log.WithField("username", _config.System.Username).Info("checking for cinammon system user")
